...
export function SiteAccount({ player }: { player?: Player }) {
  ...
  const [mode, setMode] = useState<'login'|'register'|'setup'|'reset'>(!site.initialized && site.configured ? 'setup' : 'login');
  ...
  return <div className="site-stack">
    {site.user ? <>
      ...
      {site.user.role === 'admin' && <a className="site-link" href="/admin">{t('admin')}</a>}
      ...
      <div className="site-actions" role="group" aria-label={t('account')}>{(['login','register','reset',...(!site.initialized ? ['setup'] : [])] as typeof mode[]).map(m => <Button key={m} active={mode === m} variant="soft" onClick={() => {setMode(m);setError('');}}>{t(m)}</Button>)}</div>
